...
GSE2MArrayLM <- function(GSE=NULL, filename=NULL, GPL=NULL, destdir="~/.GEO",
  ...
  if (length(gset)>1 && is.null(GPL)==FALSE) idx<-grep(GPL, attr(gset, "names")) else idx<-1
  gset <- gset[[idx]]
  ...
  for (label in varLabels(gset)) {
      if (any(grep(colname, label))){ break }
      j <- j + 1
  }
  group_cols <- gset[[j]]
  sml <- c()
  for (i in 1:length(group_cols)) {
    ...
    sml[i] <- "X"
    for (ctrl in ctrls){
      if (any(grep(ctrl, group))){
        sml[i] <- "0"
      }
    }
    for (case in cases){
      if (any(grep(case, group))){
        sml[i] <- "1"
      }
    }
  }
  print(paste(sum(sml=="1"), "control samples."))
  print(paste(sum(sml=="0"), "case samples."))
